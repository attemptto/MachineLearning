...
class CRF(Layer):
    ...
    def build(self, input_shape):
        ...
        self.chain_kernel = self.add_weight(name='chain_kernel',
        ...
        if self.use_boundary:
            self.left_boundary = self.add_weight(name='left_boundary',
                                                 shape=(self.units,),
                                                 initializer=self.boundary_initializer,
                                                 regularizer=self.boundary_regularizer,
                                                 constraint=self.boundary_constraint)
            self.right_boundary = self.add_weight(name='right_boundary',
                                                  shape=(self.units,),
                                                  initializer=self.boundary_initializer,
                                                  regularizer=self.boundary_regularizer,
                                                  constraint=self.boundary_constraint)
