...
class Decoder(nn.Module):
    ...
    def forward(self,decoder_input,hidden):
        ch_embedding = self.embedding(decoder_input)
        decoder_output, decoder_hidden = self.lstm(ch_embedding,hidden)
...
def translate(sentence):
    ...
    while True:
        ...
        pre = model.classification(decoder_output)
...
class Seq2Seq(nn.Module):
    def __init__(self,encoder_embedding_num, encoder_hidden_num, en_corpus_len, decoder_embedding_num, decoder_hidden_num, ch_corpus_len):
        ...
        self.classification = nn.Linear(decoder_hidden_num,ch_corpus_len)
    ...
    def forward(self, en_index, ch_index):
        ...
        pre = self.classification(decoder_output)
...
if __name__ == "__main__":
    ...
    ch_corpus_len = len(ch_word_2_index)+3
    en_corpus_len = len(en_word_2_index)
